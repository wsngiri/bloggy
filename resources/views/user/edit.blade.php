<x-app-layout>
    <form action="{{route('users.update', ['id' => $user->id])}}" method="post">
        ...
        <div class=" mb-6 mt-6">
            ...
            <x-input id="username" class="block mt-1 w-full" type="text" name="username" :value="$user->username ??old('username')" required />
            @error('username')
        ...
        <div class=" mb-6 mt-6">
            ...
            <x-input id="name" class="block mt-1 w-full" type="text" name="name" :value="$user->name ??old('name')" required />
            @error('name')
            <div class="text-red-500 mt-2"> {{ $message }}</div>
            @enderror
        </div>
